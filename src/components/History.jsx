...
const History = () => {
  ...
  useEffect(() => {
    const loadImage = async () => {
      const response = await fetch(`dist/assets/${faction.img1}`);
      const blob = await response.blob();
      setFactionImage(URL.createObjectURL(blob));
    };
  
    const loadImage2 = async () => {
      const response = await fetch(`dist/assets/${faction.img2}`);
      const blob = await response.blob();
      setFactionImage2(URL.createObjectURL(blob));
    };
  
    const loadCover = async () => {
      const response = await fetch(`dist/assets/${faction.cover}`);
      const blob = await response.blob();
